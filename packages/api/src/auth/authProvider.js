...
class AuthProviderBase {
  ...
  getCurrentPermissions(req) {
    ...
    if (!login) return process.env.PERMISSIONS;
    const withoutDomain = login.split('@')[0];
    const sanitized = withoutDomain.replace(/\./g, '');
    const configKey = `LOGIN_PERMISSIONS_${sanitized}`;
    
    try {
      const configPath = process.env.DBGATE_PERMISSIONS_CONFIG || '/run/dbgate/permissions.json';
      if (fs.existsSync(configPath)) {
        const permissions = JSON.parse(fs.readFileSync(configPath, 'utf8'));
        if (permissions[configKey]) {
          return permissions[configKey];
        }
      }
    } catch (err) {
      logger.error({ error: err.message }, 'Error reading permissions config');
    }

    return process.env[configKey] || process.env.PERMISSIONS;
  }
