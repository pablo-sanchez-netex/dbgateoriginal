...
module.exports = {
  ...
  async ensureOpened(conid, database, req) {
    const existing = this.opened.find(x => x.conid == conid && x.database == database);
  ...
  async queryData({ conid, database, sql }, req) {
    ...
    // Check if req is null and set authHeader accordingly
    ...
    if (authHeader) {
      ...
      try {
        ...
        loginUser = decoded?.login;
      } catch (err) {
        logger.error(extractErrorLogData(err), 'Error decoding JWT token, ignore this is run from shell command');
      }
    }

    const opened = await this.ensureOpened(conid, database, req);
    logger.info({ conid, server: opened?.connection?.server || 'unknown', database, sql, loginUser }, 'Processing query data');
  ...
  async sqlSelect({ conid, database, select }, req) {
    ...
    logger.info(
      {
        conid,
        server: opened?.connection?.server || 'unknown',
        database,
        select: { pureName: select.from?.name?.pureName || 'unknown', limit: select.range?.limit || 'unknown', offset: select.range?.offset || 'unknown' },
        loginUser,
      },
      'Processing sql select'
    );
  ...
  async runScript({ conid, database, sql, useTransaction }, req) {
    ...
    const opened = await this.ensureOpened(conid, database, req);
    logger.info({ conid, server: opened?.connection?.server || 'unknown', database, sql, loginUser }, 'Processing run script');
  ...
  async runOperation({ conid, database, operation, useTransaction }, req) {
    ...
    const opened = await this.ensureOpened(conid, database, req);
    logger.info({ conid, server: opened?.connection?.server || 'unknown', database, operation, loginUser }, 'Processing run operation');
  ...
  async collectionData({ conid, database, options }, req) {
    ...
    const opened = await this.ensureOpened(conid, database, req);
    logger.info({ conid, server: opened?.connection?.server || 'unknown', database, options, loginUser }, 'Processing collection data');
  ...
  async loadKeys({ conid, database, root, filter, limit }, req) {
    ...
    const opened = await this.ensureOpened(conid, database, req);
    logger.info({ conid, server: opened?.connection?.server || 'unknown', database, root, filter, limit, loginUser }, 'Processing load keys');
  ...
  async exportKeys({ conid, database, options }, req) {
    ...
    const opened = await this.ensureOpened(conid, database, req);
    logger.info({ conid, server: opened?.connection?.server || 'unknown', database, options, loginUser }, 'Processing export keys');
  ...
  async loadKeyInfo({ conid, database, key }, req) {
    ...
    const opened = await this.ensureOpened(conid, database, req);
    logger.info({ conid, server: opened?.connection?.server || 'unknown', database, key, loginUser }, 'Processing load key info');
  ...
  async loadKeyTableRange({ conid, database, key, cursor, count }, req) {
    ...
    const opened = await this.ensureOpened(conid, database, req);
    logger.info({ conid, server: opened?.connection?.server || 'unknown', database, key, cursor, count, loginUser }, 'Processing load key table range');
  ...
  async loadFieldValues({ conid, database, schemaName, pureName, field, search, dataType }, req) {
    ...
    const opened = await this.ensureOpened(conid, database, req);
    logger.info({ conid, server: opened?.connection?.server || 'unknown', database, schemaName, pureName, field, search, dataType, loginUser }, 'Processing load field values');
  ...
  async callMethod({ conid, database, method, args }, req) {
    ...
    const opened = await this.ensureOpened(conid, database, req);
    logger.info({ conid, server: opened?.connection?.server || 'unknown', database, method, args, loginUser }, 'Processing call method');
  ...
  async updateCollection({ conid, database, changeSet }, req) {
    ...
    const opened = await this.ensureOpened(conid, database, req);
    logger.info({ conid, server: opened?.connection?.server || 'unknown', database, changeSet, loginUser }, 'Processing update collection');
  ...
  async ping({ conid, database }, req) {
    ...
    if (existing) {
      ...
      // @ts-ignore
      existing = await this.ensureOpened(conid, database, req);
    }
  ...
  async refresh({ conid, database, keepOpen }, req) {
    ...
    await this.ensureOpened(conid, database, req);
    return { status: 'ok' };
  },
  ...
  async syncModel({ conid, database, isFullRefresh }, req) {
    ...
    testConnectionPermission(conid, req);
    const conn = await this.ensureOpened(conid, database, req);
