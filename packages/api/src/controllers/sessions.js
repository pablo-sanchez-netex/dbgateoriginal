...
module.exports = {
  ...
  async executeQuery({ sesid, sql, autoCommit }, req) {
    const session = this.opened.find(x => x.sesid == sesid);
    if (!session) {
    ...
    logger.info({ sesid, server: session?.connection?.server || 'unknown', database: session.database, sql, loginUser }, 'Execute query');
