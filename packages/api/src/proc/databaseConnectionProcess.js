...
async function handleConnect({ connection, structure, globalSettings, req }) {
  storedConnection = connection;
  lastPing = new Date().getTime();

  if(!req){
    logger.debug('No request object');
  }

  if (!structure) setStatusName('pending');
  const driver = requireEngineDriver(storedConnection);
  dbhan = await checkedAsyncCall(connectUtility(driver, storedConnection, 'app', { req }));
  logger.debug(
